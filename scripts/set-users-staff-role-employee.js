...
async function run({ dryRun = false } = {}) {
  ...
  const rolesCount = { admin: 0, employee: 0, user: 0, other: 0, nullish: 0 };
  const examples = { admin: [], employee: [], user: [], other: [], nullish: [] };

  await paginateCollection(db, "users", 1000, async (snap) => {
    for (const doc of snap.docs) {
      ...
      else if (roleRaw === "user") { rolesCount.user++; if (examples.user.length < 5) examples.user.push(doc.id); }
  ...
  const summary = { collection: "users", rolesCount, examples };
  ...
  await paginateCollection(db, "users", 1000, async (snap) => {
    for (const doc of snap.docs) {
      ...
      if (roleRaw === "admin") {
        batch.update(doc.ref, { role: "employee", isAdmin: false, updatedAt: nowIso });
        ops++; updated++;
